...
export class TaskController {
  ...
  update = (req: Request, res: Response, next: NextFunction): void => {
    try {
      ...
      const task = this.deps.updateTaskUseCase.execute(Number(req.params.id), { title, completed });
      res.status(200).json(task);
    } catch (error) {
      next(error);
    }
  ...
  complete = (req: Request, res: Response, next: NextFunction): void => {
    try {
      const task = this.deps.updateTaskUseCase.execute(Number(req.params.id), { completed: true });
      res.status(200).json(task);
    } catch (error) {
      next(error);
    }
